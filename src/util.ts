...
export class NodeGraph {
  ...
  public collapseAllLeafNodes() {
    ...
    /*
     * TODO: This can be memoized as modifying a leaf node's siblings should
     * disqualify them.  Thus by going over the inner loop for every leaf
     * node is doing extra work.
     */
  ...
  public collapseLeafNodes(parentNode?: string) {
    ...
    if ((this._graph.outEdges(parentNode) as Edge[]).length < 1) {
      return
    }
  ...
  public expandLeafNodes(parentNode?: string) {
    if (parentNode === undefined || parentNode === null) {
      return
    }
    ...
    /*
     * TODO: This can also be memoized as we are unnecessarily checking some
     * of the edges twice since collapseLeafNodes adds an invisible edge
     * between sibling nodes, thus edge (v -> w) is checked when iterating
     * over both node v, and node w.
     */
    this._graph.children(parentNode).forEach((child: string) => {
      (this._graph.nodeEdges(child) as Edge[]).forEach((sibling: Edge) => {
        if (this._graph.edge(sibling).constraint === 'false') {
          this._graph.edge(sibling).constraint = 'true'
        }

        if (this._graph.edge(sibling).style === 'invis') {
          this._graph.removeEdge(sibling)
        }
      })
    })
  }
